...
export function renderSvg(layer, box, item) {
    ...
    if (root.nodeName.toLowerCase() === 'svg') {
        ...
        [...imported.childNodes].forEach(n => { if (n.nodeType === 1) g.appendChild(document.importNode(n, true)); });
        imported = g;
    } else {
        imported = document.importNode(root, true);
    }
    ...
    wrap.setAttribute('transform', tf0);
    ...
    if (extra.length) wrap.setAttribute('transform', `${tf0} ${extra.join(' ')}`);

    alignInBox(box, wrap, item.align?.h || 'center', item.align?.v || 'middle');
    return wrap;

    function safeBBox(node) {
        try { return node.getBBox(); } catch { return { x:0, y:0, width:0, height:0 }; }
    }
}
...
export function addDeleteCross(layer, node, onClick) {
    ...
    bg.setAttribute('fill', '#ffffff'); bg.setAttribute('fill-opacity', '0.9');
    bg.setAttribute('stroke', '#ef4444'); bg.setAttribute('stroke-width', '0.3');
    g.appendChild(bg);
    ...
    const mk = (x1,y1,x2,y2) => {
        ...
        l.setAttribute('stroke', '#ef4444'); l.setAttribute('stroke-width', '0.5');
        return l;
    };
    const d = size * 0.35;
